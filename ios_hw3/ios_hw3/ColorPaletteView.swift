...
//  ios_hw3
//
//  Created by Zhasmin Mirzoeva on 17.10.2022.
//
...
final class ColorPaletteView :UIControl {
    ...
    private func setupView() {
        let redControl = ColorSliderView(colorname: "R", value: Float(chosemColor.rgba.red))
        let greenControl = ColorSliderView(colorname: "G", value: Float(chosemColor.rgba.green))
        let blueControl = ColorSliderView(colorname: "B", value: Float(chosemColor.rgba.blue))
        redControl.tag = 0
        greenControl.tag = 1
        blueControl.tag = 2
